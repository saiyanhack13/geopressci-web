...
import { useGetPressingServicesQuery, useUpdateServiceMutation, useCreateServiceMutation, useDeleteServiceMutation, PressingService } from '../../services/pressingApi';
...
const ServicesPage: React.FC = () => {
  ...
  const handleToggleActive = async (service: Service) => {
    ...
    try {
      const isAvailable = !(service.disponible ?? service.isAvailable ?? false);
      await updateService({
        serviceId,
        updates: { 
          disponible: isAvailable,
          // Préserver la compatibilité avec l'ancien format
          isAvailable 
        }
      }).unwrap();
      toast.success(`Service ${isAvailable ? 'activé' : 'désactivé'} avec succès`);
      refetch();
    } catch (error) {
