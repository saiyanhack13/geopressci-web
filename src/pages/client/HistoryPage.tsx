...
export const HistoryPage: React.FC = () => {
  ...
  const handleReorder = async (orderId: string) => {
    try {
      ...
      const reorderData = {
        pressingId: originalOrder.pressingId || 'default',
        services: originalOrder.items.map(item => ({
          serviceId: item.serviceId || item.service || 'default',
          quantite: item.quantity,
          instructions: `Service: ${item.name}`
        })),
        adresseLivraison: originalOrder.deliveryAddress || originalOrder.pressingAddress || 'Adresse par défaut',
        instructionsSpeciales: 'Recommande de la commande ' + originalOrder.orderNumber
      };

      const result = await createOrder(reorderData).unwrap();
      
      if (result) {
        toast.success('✅ Commande recréée avec succès!');
